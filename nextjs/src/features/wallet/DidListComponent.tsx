...
import { IDidList, IUpdatePrimaryDid } from "../organization/components/interfaces/organization";
...
const DIDList = ({ orgId }: { orgId: string }) => {
  ...
  const getData = async () => {
    try {
      ...
      if (data?.statusCode === apiStatusCodes.API_STATUS_SUCCESS) {
        const sortedDids = data?.data.sort((a: { isPrimaryDid: any; }, b: { isPrimaryDid: any; }) => {
  ...
  const CopyDid = ({ value, className }: { value: string, className?: string }) => (
    <Tooltip>
    ...
    <div className="w-full space-y-4">
      ...
      <div className="divide-y divide-gray-200 dark:divide-gray-700 rounded-lg border">
        {didList.map((item: IDidList, index: number) => (
          <div 
            key={item.id} 
            className={`p-4 ${item.isPrimaryDid ? 'bg-gray-100 dark:bg-gray-800' : ''}`}
          >
            <div className="flex items-center justify-between gap-4">
              ...
              {item?.did ? (
                ...
                <span className="flex-1 font-mono text-gray-500 dark:text-gray-400">
                  Not available
                </span>
              )}
