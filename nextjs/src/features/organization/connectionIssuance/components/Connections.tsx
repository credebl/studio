...
const Connections = () => {
	const [selectedConnections, setSelectedConnections] = useState< ITableData[] | ITableHtml[] >([]);
	...
	const orgDid = useSelector((state: RootState) => state.storageKeys.ORG_DID)
	const [isW3cDid, setisW3cDid] = useState<boolean>(false);
	const orgData = async () => {
		if (orgDid && (orgDid.includes(DidMethod.POLYGON) || orgDid.includes(DidMethod.KEY) || orgDid.includes(DidMethod.WEB))) {
			setisW3cDid(true)
		}
	}
	useEffect(() => {
		...
		<div className=" pt-2">
			...
			<div id="myTabContent">
				...
				>
					...
					<div className="m-4 flex items-center justify-between mb-4 pt-6">
						<h1 className="ml-1 text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">
							Selected Users
						</h1>
					</div>
					<div className="m-4 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 dark:border-gray-700 sm:p-4 dark:bg-gray-800">
						<DataTable
						...
						{selectedConnections.length ? (
							<div className="flex justify-end pt-3">
								<Button
									...
									className="text-base text-center text-black bg-primary hover:!bg-primary/90 rounded-lg hover:bg-accent-00 focus:ring-4 focus:ring sm:w-auto"
								>
									<div className="pr-3">
										<svg
