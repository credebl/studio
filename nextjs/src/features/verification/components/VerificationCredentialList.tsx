...
const VerificationCredentialList = (): JSX.Element => {
  ...
  const getproofRequestList = async (
    ...
    try {
      ...
      if (data?.statusCode !== apiStatusCodes.API_STATUS_SUCCESS) {
        setVerificationList([])
        setErrMsg('Failed to fetch proof request list.')
        return
      }

      const { totalItems, nextPage, lastPage, data: requests } = data.data

      setPageInfo({ totalItem: totalItems, nextPage, lastPage })
      setTotalItem(totalItems)

      const credentialList = requests?.map((requestProof: RequestProof) => {
        const { presentationId, connectionId, createDateTime, state } =
          requestProof
        ...
        const stateLabel =
          {
            ...
            [ProofRequestState.done]: ProofRequestStateUserText.done,
            [ProofRequestState.abandoned]: ProofRequestStateUserText.abandoned,
          }[state] ?? ''

        const stateClasses =
          {
            [ProofRequestState.requestSent]: 'text-done border',
            [ProofRequestState.done]: 'text-success border',
            [ProofRequestState.abandoned]: 'text-destructive border',
            [ProofRequestState.presentationReceived]:
        ...
        return {
          data: [
            { data: presentationId || 'Not available' },
            { data: connectionId || 'Not available' },
            { data: formattedDate },
            {
              data: (
                <span
                  className={`${stateClasses} flex justify-center rounded-md py-0.5 text-xs font-medium min-[320px]:w-full min-[320px]:px-1 sm:mr-0 sm:px-0 md:mr-2 lg:px-0.5 2xl:w-8/12`}
                >
                  {stateLabel}
                </span>
              ),
            },
            {
              data: (
                <Button
                  ...
                  onClick={() => {
                    openProofRequestModel(true, presentationId, state)
                    getProofPresentationData(presentationId)
                  }}
                >
                  {state === ProofRequestState.done ? (
