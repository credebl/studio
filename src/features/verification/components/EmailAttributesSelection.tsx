...
const EmailAttributesSelection = (): JSX.Element => {
  ...
  // Extract attribute processing for a single schema
  const processSchemaAttributes = (
    schema: any,
    selectedCredDefs: any[],
  ): any[] => {
    const attributes = []
    ...
    for (const attribute of schema.attributes) {
      for (const credDef of matchingCredDefs) {
        attributes.push({
          displayName: attribute.displayName,
          attributeName: attribute.attributeName,
          isChecked: false,
  ...
  // Extract the W3C schema processing logic
  const processW3CSchemas = (
    parsedSchemaDetails: any[],
    selectedCredDefs: any[],
  ): any[] => {
    const allAttributes = []

    for (const schema of parsedSchemaDetails) {
      if (schema.attributes && Array.isArray(schema.attributes)) {
        const schemaAttributes = processSchemaAttributes(
          schema,
          selectedCredDefs,
        )
        allAttributes.push(...schemaAttributes)
      }
    }
  ...
  const loadAttributesData = async (): Promise<void> => {
    ...
    try {
      ...
      if (!w3cSchema) {
        allAttributes = parsedSchemaDetails.flatMap((schema) =>
          getAttributesFromSchema(schema, selectedCredDefs),
        )
      } else {
        allAttributes = processW3CSchemas(parsedSchemaDetails, selectedCredDefs)
      }
