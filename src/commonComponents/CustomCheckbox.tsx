...
const CustomCheckbox: React.FC<ICustomCheckboxProps> = ({ showCheckbox, isVerificationUsingEmail, onChange, schemaData }) => {
  ...
  useEffect(() => {
    if (schemaData) {
      try {
        const selectedSchemas = (localStorage.getItem('selectedSchemas') ?? '[]');
        if(selectedSchemas){
          const abcd = JSON.parse(selectedSchemas)
          const isChecked = abcd.some((schema: ISchemaData) => schema.schemaId === schemaData.schemaId);
