...
export const onRequest = async (context: any, next: any) => {
  ...
  response.headers.set('Content-Security-Policy',`default-src 'self'; script-src 'self' ${allowedDomain} 'nonce-${nonce}_scripts'; style-src 'self' ${allowedDomain}; font-src ${allowedDomain}; img-src 'self' ${allowedDomain}; frame-src 'self' ${allowedDomain}; object-src 'none'; media-src 'self'; connect-src 'self' ${allowedDomain}; form-action 'self'; frame-ancestors 'self'; `);
  ...
  // .split("style=\"").join(` nonce="${nonce}_styles" style="`).split("<style").join(`<style nonce="${nonce}_styles" `);
