...
const AllSchemasList = (props: { schemaSelectionCallback: (schemaId: string, schemaDetails: SchemaDetails) => void; }) => {
    ...
    const [filterEndorsement, setFilterEndorsement] = useState<boolean | null>(null);

    const getEcosystems =async () => {
        const ecosystems = "Eco1";
        setSelectedEcosystem(ecosystems);
    }


	const getSchemaList = async (schemaListAPIParameter: GetAllSchemaListParameter, filterEcosystem) => {
		try {
	...
	useEffect(() => {
		getSchemaList(schemaListAPIParameter, filterEndorsement)

	}, [schemaListAPIParameter, filterEndorsement])
	...
	const handleFilter = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (e.target.value === 'Ecosystem Schemas') {
            getEcosystems();
        }
		else {
    ...
    const handleEcosystemFilter = (e: React.ChangeEvent<HTMLSelectElement>) => {
        console.log("Handle Ecosystem Filter", e.target.value)
					...
					<div className="flex flex-col items-center justify-between mb-4 pr-4 sm:flex-row">
						<div id='schemasSearchInput' className='mb-2 pl-0 sm:pl-2 flex space-x-2 items-end'>
							...
							<select onChange={handleFilter} id="schamfilter"
								...
								<option selected>Ecosystem Schemas</option>
                                <option >Organization's Schemas</option>
                        ...
                        {selectedEcosystem && (
                            <select onChange={handleEcosystemFilter} id="ecosystemFilter"
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-11">
