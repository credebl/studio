...
console.log("credentialList",verificationList);
				...
				const credentialList = data?.data?.map((requestProof: RequestProof) => {
					...
					return {
						data: [
							...
							{
								data: <Button
									disabled={!(requestProof.state === ProofRequestState.presentationReceived) && requestProof?.state !== 'done'}
									...
									onClick={() => {
										openProofRequestModel(true, requestProof?.id, requestProof?.state),getProofPresentationData(requestProof?.id,)
									}}
								>
								{requestProof?.state=== "done" ? 
								<p className='pr-1 flex text-center justify-center item-center'>	<svg className='pr-1 mt-0.5 flex items-center' xmlns="http://www.w3.org/2000/svg" width="24" height="15" fill="none" viewBox="0 0 24 15">  <path fill="#fff" d="M23.385 7.042C23.175 6.755 18.165 0 11.767 0 5.37 0 .36 6.755.15 7.042a.777.777 0 0 0 0 .916C.36 8.245 5.37 15 11.767 15c6.398 0 11.408-6.755 11.618-7.042.2-.273.2-.643 0-.916Zm-11.618 6.406c-4.713 0-8.795-4.483-10.003-5.949 1.207-1.466 5.28-5.947 10.003-5.947 4.713 0 8.794 4.482 10.003 5.949-1.207 1.466-5.28 5.947-10.003 5.947Z"/>  <path fill="#fff" d="M11.772 2.84a4.66 4.66 0 0 0-4.655 4.655 4.66 4.66 0 0 0 4.655 4.655 4.66 4.66 0 0 0 4.656-4.655 4.66 4.66 0 0 0-4.656-4.655Zm0 7.758A3.107 3.107 0 0 1 8.67 7.495a3.107 3.107 0 0 1 3.103-3.103 3.107 3.107 0 0 1 3.104 3.103 3.107 3.107 0 0 1-3.104 3.103Z"/></svg>
						<span className='pl-1'>View</span> </p>
							:

								 
							<p className='flex items-center justify-center'>		<svg className='pr-2 flex items-center' xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="none" viewBox="0 0 17 17">
