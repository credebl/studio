...
const VerificationDashboard = () => {
	const options = [
		...
		},
		{
			...
			path: pathRoutes.organizations.verification.bulkIssuance,
		},
	];
	return (
		<div className="px-4 pt-2 h-full h-[700px]">
			...
			<div className="px-6 pt-6 bg-white border border-gray-200 rounded-lg shadow-sm 2xl:col-span-2 dark:border-gray-700 dark:bg-gray-800">
				<p className="text-gray-900 text-xl text-start font-medium dark:text-white">
					Select the appropriate action for verifying credential(s){' '}
				</p>
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-16 pt-12">
					{options.map((option) => (
						<Card
							key={option.heading}
							className={`${option.heading === 'Connection' ? 'custom-card group transform transition duration-500 ease-in-out hover:scale-105 cursor-pointer overflow-hidden overflow-ellipsis dark:hover:bg-primary-700 hover:bg-primary-700 border border-gray-200 shadow-md dark:border-gray-600 dark:bg-gray-700' : 'opacity-50 bg-gray-300'}`}
							style={{
								maxHeight: '100%',
								overflow: 'auto',
								height: '168px',
							...
							onClick={() => option.heading==='Connection' && (window.location.href = option?.path)}
						>
							<div
