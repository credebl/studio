    ...
    return (
        <section className="pt-2 px-4">
            ...
            <div className="flex items-center justify-between mb-4 ">
                <h1 className='text-blue-700 font-semibold text-2xl'>Organization(s) Details</h1>
                ...
                <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-gray-500 dark:text-white">
                            ...
                            <tbody>
                                {data.map((item) => (
                                    <tr key={item.name} className="dark:text-white border-b border-blue-500 last:border-0">
                                        <th scope="row" className="px-4 py-3 font-medium text-gray-600 whitespace-nowrap dark:text-white">{item.name}</th>
                                        <td className="px-4 py-3 text-gray-600 dark:text-white">{item.plan}</td>
                                        <td className="px-4 py-3 text-gray-600 dark:text-white">{item.role}</td>
                                        <td className="px-4 py-3 text-gray-600 dark:text-white">
                                            <a href={`${pathRoutes.subscription.organization}/${item.name}`}>
                                                <svg className="h-4 w-4 text-gray-600 dark:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
