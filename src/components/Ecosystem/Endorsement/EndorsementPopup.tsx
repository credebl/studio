...
import { DidMethod, EndorsementStatus, EndorsementType } from '../../../common/enums';
import {
	checkEcosystem,
	getEcosystemId,
	getOwnerAdminRole,
...
const EndorsementPopup = (props: {
	...
	useEffect(() => {
		if (props.openModal) {
			if (endorserDid.includes(DidMethod.INDY)) {
				setIsW3CDid(false);
			} else if (endorserDid.includes(DidMethod.POLYGON) || endorserDid.includes(DidMethod.KEY) || endorserDid.includes(DidMethod.WEB)) {
				setIsW3CDid(true);
			}
		}
	}, [props.openModal, endorserDid]);
