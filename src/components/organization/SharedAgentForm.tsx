...
import AlreadyDidSharedForm from './AlreadyDidSharedForm';
	...
	return (
		<div className="mt-4 max-w-lg flex-col gap-4">
			...
			{!haveDidShared ? (
				...
				<AlreadyDidSharedForm
						orgName={orgName}
						submitSharedWallet={submitSharedWallet}
						selectDidMethod={(e, formikHandlers) => selectDidMethod(e, formikHandlers)}
						generatePolygonKeyValuePair={generatePolygonKeyValuePair}
						generatedKeys={generatedKeys}
						privateKeyValue={privateKeyValue}
						setPrivateKeyValue={(val) => setPrivateKeyValue(val)}
						domainValue={domainValue}
						setDomainValue={(val) => setDomainValue(val)}
						mappedData={mappedData}
						selectDidLedger={(e, formikHandlers) => selectDidLedger(e, formikHandlers)}
						selectedNetwork={selectedNetwork}
						selectedDid={selectedDid}
						setSelectedDid={(val) => setSelectedDid(val)}
						showMethod={(val) => showMethod(val)} 
