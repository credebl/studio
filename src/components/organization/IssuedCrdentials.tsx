    ...
    const getIssuedCredDefs = async () => {
        ...
        if (data?.statusCode === apiStatusCodes.API_STATUS_SUCCESS) {
            ...
            const credentialList = data?.data?.organizations.map((issuedCredential: Organisation) => {
                return [{ data: issuedCredential.website ? issuedCredential.website : 'Not available', subData: 'Did:Indy:bcovrin' }, { data: issuedCredential.name }, { data: dateConversion(issuedCredential.lastChangedDateTime) },
                {
                    data: <span
                        className={`bg-cyan-100 ${issuedCredential.website === IssueCredential.offerSent && 'text-blue-900'} ${issuedCredential.website === IssueCredential.credentialIssued && 'text-green-900'} text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md dark:bg-gray-700 dark:text-white border border-cyan-100 dark:border-cyan-500`}                    >Offer Sent</span>
                }, {
