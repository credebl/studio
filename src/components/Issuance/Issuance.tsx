...
const IssueCred = () => {
	...
	const createIssuanceForm = (selectedUsers: SelectedUsers[], attributes: string[], credDefId: string, orgId: number) => {
	...
	const getSchemaDetails = async (schemaId: string, orgId: number): Promise<string[] | null> => {
	...
	return (
		<div className="px-4 pt-6">
			...
			{!schemaLoader ?
				<Card  style={{ width: '470px', height: '140px', maxWidth: '100%', maxHeight: '100%', overflow: 'auto' }}>
													...
													{values.attributes.map((attr, index) => (
														<div>
															<div key={index} className="flex">
																<label htmlFor={`attributes.${index}.value`} className='w-1/3 pr-2 flex justify-end items-center font-light'>{attr.name}</label>
																<Field
																	type="text"
															...
															<div className="flex">
																...
																<ErrorMessage className='text-red-500 text-xs w-3/4' name={`attributes.${index}.value`} component="div" />
															</div>
														</div>
													))}
												</div>
